...
output_file = "/Users/aditya1981/Documents/Unified Data Ingestion Engine/data/processed/20240212/AL1_SOLEXS_SDD2_L1_advanced_fit.png"

def find_peak_bin(file_path):
    print("Scanning .pi file for peak in 03:23-04:10 window...")
    try:
        with fits.open(file_path) as hdul:
            ...
            # constrain to 03:23:00 (12180) to 04:10:00 (15000)
            start_idx = 12180
            end_idx = 15000
            
            flare_counts = total_counts[start_idx:end_idx]
            best_bin = np.nanargmax(flare_counts) + start_idx + 1  # 1-indexed
            max_c = np.nanmax(flare_counts)
...
def run():
    ...
    import os
    os.chdir(os.path.dirname(pi_file))
    fname = os.path.basename(pi_file)
    s = xspec.Spectrum(f"{fname}{{{peak_bin}}}")
    s.response = rmf_file
    ...
    best_MK = (10 ** best_logT) / 1e6
    best_kT = (10 ** best_logT) / (1.16045e7)
    ...
    ax1.set_title(f'SoLEXS Flare Spectral Fit (12 Feb 2024, Peak Bin: {peak_bin})', fontsize=16, fontweight='bold', pad=15)
    ...
    # Save a copy to the artifacts directory so I can display it
    import shutil
    art_dir = "/Users/aditya1981/.gemini/antigravity-ide/brain/ead5211d-dfba-45f4-a3a2-adaf18c4ec59"
    shutil.copy2(output_file, os.path.join(art_dir, "AL1_SOLEXS_SDD2_L1_advanced_fit_20240212.png"))
