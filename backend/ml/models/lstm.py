...
class LSTMModel(BaseScientificModel):
    def __init__(self, hidden_dim: int = 24, output_dim: int = 5, is_classifier: bool = True):
    ...
    def train(self, X: np.ndarray, y: np.ndarray, **kwargs) -> Dict[str, Any]:
        ...
        final_loss = self.model.fit(X, y, epochs=150, lr=0.02)
        return {"loss": final_loss, "n_samples": len(X), "network": "LSTM", "parameters_count": input_dim * self.hidden_dim + self.hidden_dim * self.output_dim}
