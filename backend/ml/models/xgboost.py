...
class XGBoostModel(BaseScientificModel):
    def __init__(self, n_estimators: int = 15, learning_rate: float = 0.1, output_dim: int = 5, is_classifier: bool = True):
    ...
    def train(self, X: np.ndarray, y: np.ndarray, **kwargs) -> Dict[str, Any]:
        ...
        return {"loss": final_loss, "n_samples": len(X), "input_dim": input_dim}
    ...
    def evaluate(self, X: np.ndarray, y: np.ndarray) -> Dict[str, Any]:
        ...
        if self.is_classifier:
            ...
            # Calculate a basic log loss / cross entropy
            eps = 1e-15
